...
var times = [[-0.83, 'sunrise', 'sunset'], [-0.3, 'sunriseEnd', 'sunsetStart'], [-6, 'dawn', 'dusk'],
...
var getTimes = function (date, lat, lng) {
    ...
    var result = { solarNoon: new Date((Jnoon + 0.5 - J1970) * dayMs), nadir: new Date((Jnoon - 0.5 + 0.5 - J1970) * dayMs) };
...
function getAstroData(pos) {
    ...
    var sunrise = formatTime(sunTimes.sunrise);
    var sunset = formatTime(sunTimes.sunset);
    var moonmetrics = getMoonIllumination(d);
    var moonphase = Math.round(moonmetrics.phase * 28);

    var dictionary = {
        "HourSunrise": sunrise.int,
        "HourSunset": sunset.int,
        "WEATHER_SUNSET_KEY": sunset.str24h,
        "WEATHER_SUNRISE_KEY": sunrise.str24h,
        "WEATHER_SUNSET_KEY_12H": sunset.str12h,
...
function fetchWeatherData(pos) {
    ...
    if (weatherprov === 'ds') { // Open-Meteo
        ...
        parseFunc = function (json) {
            ...
            var forecast_high_tempf = Math.round((json.daily[0].temp.max * 1.8) - 459.67);
            var forecast_low_tempf = Math.round((json.daily[0].temp.min * 1.8) - 459.67);
            var forecast_high_tempc = Math.round(json.daily[0].temp.max - 273.15);
            var forecast_low_tempc = Math.round(json.daily[0].temp.min - 273.15);
            var high = String(temptousewu(units, forecast_high_tempf, forecast_high_tempc));
            var low = String(temptousewu(units, forecast_low_tempf, forecast_low_tempc));
            var highlow = high + '|' + low;
